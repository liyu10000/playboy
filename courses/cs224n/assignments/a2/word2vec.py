...
def naiveSoftmaxLossAndGradient(
    ...
    yhat[outsideWordIdx] -= 1  # yhat now is yhat - y
    gradCenterVec = np.dot(outsideVectors.T, yhat)
    gradOutsideVecs = np.outer(yhat, centerWordVec)  # makes (nx1, dx1) to be nxd
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    dotproduct = np.dot(negSampleVectors, centerWordVec)
    ac = sigmoid(dotproduct[0])
    ak = sigmoid(-dotproduct[1:])
    loss = -np.log(ac) - np.sum(np.log(ak))
    gradCenterVec = -(1 - ac) * negSampleVectors[0] + np.dot(negSampleVectors[1:, :].T, (1 - ak))
    gradOutsideVecs = np.zeros_like(outsideVectors)
    gradOutsideVecs[outsideWordIdx] = - (1 - ac) * centerWordVec
    gradOutsideVecs[indices[1:], :] = np.outer((1 - ak), centerWordVec)
    ### END YOUR CODE
