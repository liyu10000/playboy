...
def minibatch_parse(sentences, model, batch_size):
    ...
    window = list(range(min(len(sentences), batch_size)))
    next_i = min(len(sentences), batch_size)
    while sum(window) > -len(window):
        batch_parses = [partial_parses[w] for w in window if w != -1]
        ...
        for i,w in enumerate(window):
            if w == -1:
                continue
            pp = partial_parses[w]
            pp.parse_step(transitions[count])
            count += 1
            if pp.finished():
                dependencies[w] = pp.dependencies
                window[i] = next_i if next_i < len(sentences) else -1
                next_i += 1
    ### END YOUR CODE
