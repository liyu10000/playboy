...
def rnn_backward(dh, cache):
    ...
    ##############################################################################
    N, T, H = dh.shape
    dx, dh_, dWx, dWh, db = [], 0, 0, 0, 0
    for i in range(T-1, -1, -1):
        dh_ += dh[:, i, :]  # gradient from upstream plus that from backward
        cache_ = cache[i]
        dx_, dh_, dWx_, dWh_, db_ = rnn_step_backward(dh_, cache_)
        dx.append(dx_)
        dWx += dWx_
        dWh += dWh_
        db += db_
    dx.reverse()
    dx = np.asarray(dx).transpose((1, 0, 2))
    dh0 = dh_
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (next_c, i, f, o, g, x, prev_h, prev_c, Wx, Wh)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    next_c, i, f, o, g, x, prev_h, prev_c, Wx, Wh = cache
...
def lstm_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dh_ += dh[:, t, :]
        cache_ = cache[t]
        dx_, dh_, dc_, dWx_, dWh_, db_ = lstm_step_backward(dh_, dc_, cache_)
        dx.append(dx_)
        dWx += dWx_
        dWh += dWh_
        db += db_
